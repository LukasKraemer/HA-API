...
class HaTool:
    ...
    def _getMissiongSummaryTrips(self):
        return pd.read_sql_query(f'''SELECT DISTINCT trip_number +1
                                    ...
                                    WHERE trip_number +1 NOT IN (SELECT DISTINCT trip_number FROM {self._overview_table});''',
                                 con=self._engine)
    ...
    def start(self, program):
        ...
        raw_data_table = self._raw_data_table
        overview_table = self._overview_table
        process_running = []
        if program == "trips":
